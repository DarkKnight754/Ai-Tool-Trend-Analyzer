...
def keyword_recommend(task: str, available_tools: List[Dict]) -> Dict:
    ...
    if scores[best_cat] == 0:
        best_cat = "Chatbot & Assistant"  # sensible default

    # Find tools in that category
    matched = [t for t in available_tools if t.get("category") == best_cat]
    if not matched:
        matched = available_tools  # fallback to all

    recommended = matched[0]
    alternative = matched[1] if len(matched) > 1 else available_tools[0]

    return {
        "recommended_tool": recommended["name"],
        "reason": f"{recommended['name']} falls under '{best_cat}' which best matches your task. {recommended.get('summary', recommended.get('description',''))[:150]}",
    ...
    for attempt in range(3):  # retry up to 3 times
        try:
            ...
            if "429" in err or "rate_limit" in err:
                ...
                return "{}"
    print("[LLM] All retries exhausted — using keyword fallback")
    return "{}"
...
def recommend_tool_for_task(task: str, available_tools: List[Dict]) -> Dict:
    ...
    if not client:
        print("[LLM] No Groq key — using keyword-based recommendation")
...
"""
    ...
    # If LLM returned empty/bad JSON, fall back to keyword
    if not result or not result.get("recommended_tool"):
        print("[LLM] LLM returned empty result — using keyword fallback")
        return keyword_recommend(task, available_tools)
